...
def save_export_summary(
    ...
    payload = {
        ...
        "spreadsheet_name": str(spreadsheet_name or SPREADSHEET_NAME),
    }
...
def _parse_sheet_match_date(value: str) -> date | None:
    text = (value or "").strip()
    if not text:
        return None
...
def get_latest_match_date_from_sheet() -> str:
    ...
    spreadsheet = open_or_create_spreadsheet(client, sheet_id)
    worksheet = get_matches_worksheet(spreadsheet)

    # Column B contains match date values.
    date_values = worksheet.col_values(2)
    parsed_dates = [_parse_sheet_match_date(raw) for raw in date_values[1:]]
    valid_dates = [item for item in parsed_dates if item is not None]

    if not valid_dates:
        raise RuntimeError("Не удалось определить дату последнего матча в Google Sheets")
...
def open_or_create_spreadsheet(
    ...
    target_id = (spreadsheet_id or os.getenv("GOOGLE_SHEET_ID", "")).strip()
    if not target_id:
        raise RuntimeError("Переменная окружения GOOGLE_SHEET_ID не задана")

    logging.info("Открытие Google Sheets по GOOGLE_SHEET_ID")
...
@google_api_retry()
def append_rows_single_batch(rows: list[list[Any]]) -> dict[str, int | str]:
    creds_path = (GOOGLE_CREDS_PATH or "").strip()
    if not creds_path:
        raise RuntimeError("Переменная окружения GOOGLE_CREDS_PATH не задана")
    ...
    client = build_gspread_client(str(creds_file))
    spreadsheet = open_or_create_spreadsheet(client, os.getenv("GOOGLE_SHEET_ID"))
    worksheet = get_matches_worksheet(spreadsheet)
    ...
    duplicates_removed = deduplicate_matches_worksheet(worksheet)
...
@google_api_retry()
def sync_dashboard_only() -> dict[str, int | str]:
    creds_path = (GOOGLE_CREDS_PATH or "").strip()
    if not creds_path:
        raise RuntimeError("Переменная окружения GOOGLE_CREDS_PATH не задана")
...
def main() -> int:
    ...
    if not new_files:
        ...
        if latest_file is not None:
            ...
            try:
                summary = append_rows_single_batch(rows_for_refresh)
            except Exception as exc:
            ...
            save_export_summary(
                ...
                incoming_rows=int(summary.get("incoming_rows", 0)),
                added_rows=int(summary.get("added_rows", 0)),
        ...
        try:
            sync_summary = sync_dashboard_only()
        except Exception as exc:
        ...
        save_export_summary(
            mode="sync_only",
            status="ok",
            duplicates_removed=int(sync_summary.get("duplicates_removed", 0)),
    ...
    try:
        summary = append_rows_single_batch(rows)
    except Exception as exc:
